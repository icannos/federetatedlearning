...
datasets = [(X[i], Y[i]) for i in range(nb_agent)]
...
plt.savefig(f"img/unbalanced-{optimizer}-{fusion_mode}_mnist_collab_vs_alone-{epochs}-{batch_size}-{steps_by_step}.svg", format="svg")
d.to_csv(f"benchmarks/unbalanced-{optimizer}-{fusion_mode}_benchmark_mnist-{epochs}-{batch_size}-{steps_by_step}.csv")

# Change the path to where you want, it will dump the model at the end in order to allow us to make test on it
master.save("models/unbalanced_mnist")
